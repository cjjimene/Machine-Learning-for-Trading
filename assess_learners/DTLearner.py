...
class DTLearner(object):
    ...
    def build_tree(self, data):
        ...
        if np.all(data[:-1] == data[0, -1]):
            ...
            correlation = np.empty(data.shape[1] - 1)
            for x in range(data.shape[1] - 1):
                correlation[x] = abs(np.corrcoef(data[:, x], data[:, -1])[0, 1])
            # If correlation is the same for all factors, randomly select one.
            if np.all(correlation == correlation[0]):
                i = random.choice(range(len(correlation)))
            else:
                i = np.argmax(abs(correlation))
